-- =============================================================================
-- 0010_app_sync_records
-- =============================================================================
-- Generic local-first document sync target for the desktop app.
--
-- The active hosted schema intentionally contains normalized product tables,
-- while Dexie stores the full local-first app model. `app_sync_records` lets
-- the desktop sync_queue safely persist every local mutation without requiring
-- a brittle one-table-to-one-table mapping for every UI feature.
...
create policy "own app sync records" on public.app_sync_records
  for all
