...
test('builds a manifest from signed platforms and skips unsigned artifacts', async () => {
  ...
  try {
    await writeFile(path.join(assetsDir, 'Jarvis-One-0.1.25-Windows-x64.exe'), 'installer');
    await writeFile(path.join(assetsDir, 'Jarvis-One-0.1.25-Windows-x64.exe.sig'), 'signature');
    ...
    assert.equal(
      ...
      'https://example.test/releases/v0.1.25/Jarvis-One-0.1.25-Windows-x64.exe',
    );
  } finally {
