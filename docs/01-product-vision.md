# VibeSpace - Product Vision & Executive Overview
...
**VibeSpace is the unification.** One desktop-and-web app where every model, every agent, every voice, and every task lives under one persistent memory and one always-available assistant.

We are building what BridgeMind hinted at and what no shipping product has actually delivered: a **multi-agent operating layer** for everyday work, with a voice-first overarching assistant ("VibeSpace") that ties all the agents, tools, conversations, schedules, and reminders together.

## 2. North-star user story

> Sarah, a product manager, says "Hey VibeSpace" while making coffee.
>
...
>
> VibeSpace pulls the meeting transcript from her unified memory, fans out to a research agent for context, summarizes the three questions on her glass-edge HUD, drops a task into her live to-do list with a 9:45 AM reminder, and gently notifies her at exactly 9:43 AM with the prepped context already loaded.
>
...
VibeSpace is built on five pillars. Every feature must reinforce at least two of them.
...
3. **VibeSpace voice layer.** Always-available voice assistant with wake word, push-to-talk, low-latency S2S option, and natural-language access to every other surface in the app.
4. **Live to-do list + smart scheduler.** A first-class task system where VibeSpace creates, modifies, schedules, snoozes, and reminds via OS notifications. The to-do list is a native peer to chat, not a plug-in.
...
| | BridgeMind | Cursor | Lindy | Granola | ChatHub | **VibeSpace** |
|---|---|---|---|---|---|---|
...
2. **Mobile companion (iOS + Android, React Native + Expo):** Read tasks, dictate to VibeSpace, receive reminders, approve agent actions. Not a full chat client.
3. **Browser extension (Chrome + Firefox + Safari):** Capture pages into memory, summon VibeSpace, sidebar chat.
4. **Menu-bar/system-tray micro-app (Mac + Win):** Always-on VibeSpace presence, push-to-talk hotkey, floating to-do drawer, notification center.
5. **Web app (vibespaceos.com):** Cloud sync surface, shareable chats, account settings. Not the primary surface - desktop is.
...
- **Voice-driven create/modify.** "Hey VibeSpace, add 'review PR #1234' due Friday at 4pm, high priority, link to the Cursor thread."
...
- **Smart scheduling.** VibeSpace picks optimal reminder times using calendar density, location, quiet hours, and deadline pressure - not just static "remind at X."
...
- **Snooze with intelligence.** "Snooze until I'm done with my next meeting" - VibeSpace figures out when that is.
- **Daily plan briefing.** Morning standup with VibeSpace: today's tasks, conflicts, suggestions, drafted prep.
...
4. **Branding.** VibeSpace is the public product name; VibeSpaceOS is the ecosystem and web home at vibespaceos.com.

## 9. Non-goals

To stay focused, VibeSpace explicitly **does not** try to be:
