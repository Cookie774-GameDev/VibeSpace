...
export function buildAddendumText(): string {
  ...
  return [
    ...
    '- For "open 5 terminals with opencode" (or any bulk count), use `terminal.bulkOpen` with params like `{"count":5,"command":"opencode"}` — the user must click **Approve** before panes open.',
