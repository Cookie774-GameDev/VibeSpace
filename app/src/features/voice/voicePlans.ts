...
export const VOICE_PLANS: Record<VoicePlanId, VoicePlanInfo> = {
  ...
  starter: {
    ...
    label: 'Starter',
    priceUsd: 10,
    callVoiceBudgetUsd: 2.17,
    cloudSecondsMax: seconds(2.17),
  },
  pro: {
    id: 'pro',
    label: 'Pro',
    priceUsd: 50,
    callVoiceBudgetUsd: 10.85,
    cloudSecondsMax: seconds(10.85),
  },
  ultra: {
    id: 'ultra',
    label: 'Ultra',
    priceUsd: 100,
    callVoiceBudgetUsd: 21.7,
    cloudSecondsMax: seconds(21.7),
  },
  apex: {
    id: 'apex',
    label: 'Supernova',
    priceUsd: 200,
    callVoiceBudgetUsd: 43.4,
    cloudSecondsMax: seconds(43.4),
  },
};
...
export const VOICE_PROVIDERS: Record<VoiceProviderId, VoiceProviderInfo> = {
  jarvis_local: { id: 'jarvis_local', label: 'Jarvis High — Local Piper', cloud: false },
...
export function usageCopy(plan: VoicePlanId, usedSeconds: number, limitSeconds: number): string {
  if (plan === 'free') {
    return `Local Jarvis High voice included. Launch Deepgram: ${DEEPGRAM_LAUNCH_PROMO.free.minutesLabel} one-time cloud voice trial.`;
  }
  if (plan === 'starter') {
    return `Cloud voice: ${fmtMinutes(usedSeconds)} used / ${fmtMinutes(limitSeconds)} included. Local Jarvis High voice unlimited.`;
  }
  // pro / ultra shown in hours
  return `Cloud voice: ${fmtHours(usedSeconds)} used / ${fmtHours(limitSeconds)} included. Local Jarvis High voice unlimited.`;
}
...
export function deepgramPromoCopy(
  ...
  if (!poolActive) {
    return 'Launch Deepgram promo is paused. Local Jarvis High voice still included.';
  }
  const promo = DEEPGRAM_LAUNCH_PROMO[plan];
  const remaining = Math.max(0, limitSeconds - usedSeconds);
  if (remaining <= 0) {
    return plan === 'free'
      ? 'Launch Deepgram trial used. Local Jarvis High voice still included.'
...
export const FALLBACK_MESSAGES = {
  quotaExceeded:
    "You've used your monthly cloud voice time. I switched to free local Jarvis High voice.",
  cloudFailure: 'Cloud voice is temporarily unavailable. I switched to local Jarvis High voice.',
